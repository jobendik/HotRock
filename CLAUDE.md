...
## Current status
> Keep this line current as you progress. **Vertical slice COMPLETE — M0–M6 all landed and playable vs 8 bots**, plus a **deploy + mobile pass**: GitHub Pages CI (`deploy.yml`, relative `base: './'`), installable PWA (manifest + icon, `display: fullscreen`), a **floating thumb-joystick** (left-half touch zone), screen wake-lock, haptics (from local store transitions), pause-on-hidden (`intent:pause/resume`), per-tier particle quality (touch defaults to medium), a Settings fullscreen toggle, and touch CSS hardening. M6 added: Results (winner + your run + rewarded double-payout) + Play Again/requeue, Settings (sound/volume/quality, persisted), zero-asset WebAudio SFX, screen shake + `prefers-reduced-motion`, ad hooks via a `GameServices` seam (ui never imports platform), and a UI/layer-decoupling test. Also a **hot-potato balance pass** driven by a headless harness (`npm run balance`): carrier keeps full speed + a pickup grace window + per-rammer steal cooldown + committed-ram steal speed, and bots now cap chasers (no dogpile), pick the safest dock, juke past threats, and ease in to park-and-extract — extraction went 0% → ~98% with ~18 hand-offs/round (was 75). 55 Vitest tests (incl. a balance-regression guard); typecheck + lint + test + build + dev all green. **Next: P7 — authoritative multiplayer per `docs/NETCODE.md`.**
